...
namespace pico_ssd1306 {
    ...
    void drawInvText(pico_ssd1306::SSD1306 *ssd1306, const unsigned char *font, const char *text, uint8_t anchor_x,
                    uint8_t anchor_y, WriteMode mode, Rotation rotation) {
        if(!ssd1306 || !font || !text) return;
        ...
        while (text[n] != '\0') {
            switch (rotation) {
                case Rotation::deg0:
                    drawInvChar(ssd1306, font, text[n], anchor_x + (n * font_width), anchor_y, mode, rotation);
                    break;
                case Rotation::deg90:
                    drawInvChar(ssd1306, font, text[n], anchor_x, anchor_y + (n * font_width), mode, rotation);
                    break;
            }
    ...
    void drawInvChar(pico_ssd1306::SSD1306 *ssd1306, const unsigned char *font, char c, uint8_t anchor_x, uint8_t anchor_y,
                     WriteMode mode, Rotation rotation) {
        if(!ssd1306 || !font || c < 32) return;
